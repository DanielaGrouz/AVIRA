  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (!currentId) {
        setMessage({
          type: 'error',
          text: 'System Error: User ID not found. Please log out and log in again.',
        });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.phoneNumber) {
      ...
      if (!phoneRegex.test(formData.phoneNumber)) {
        setMessage({
          type: 'error',
          text: 'Invalid phone number. Must be exactly 10 digits starting with 05.',
        });
        return;
      }
    }
    ...
    try {
      ...
      const newPictureUrl = updatedUser.picturePath
        ? `${updatedUser.picturePath}?t=${new Date().getTime()}`
        : avatarUrl;
      ...
      if (selectedFile) setAvatarUrl(newPictureUrl);
      setSelectedFile(null);
    } catch (err) {
      console.error('Error updating profile:', err);
      const errorMsg =
        err.data?.error?.message || err.message || 'Failed to update profile. Please try again.';
